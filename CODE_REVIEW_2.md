...
|------|------------------|----------------|
| 流式展示 | ✅ `_responseBuffer` 累积 | ✅ 主回复逻辑正确；thought 子流仍有问题 |
| 连接重试 | ✅ `catch` 中 `_acp = undefined` | ❌ **`catch` 实际不会执行**，重试仍未生效 |
| New Chat placeholder | ✅ 重新 getElementById | ❌ **`const`  reassignment 运行时错误** |
| 进程 exit 清理 | ✅ exit 时 `stop()` | ⚠️ 部分清理；Provider 仍持有 dead `_acp`；status 竞态 |
| 取消生成 | ✅ ⏹ 按钮 | ⚠️ UI/状态未闭环 |
| 权限 UI | ✅ Allow/Deny 弹窗 | ⚠️ 字段、选项映射、Deny 语义均有缺陷 |
| 架构 / Phase 2+ | — | ⚠️ 多项设计缺口未解决 |
...
| # | 原问题 | CR #1 状态 | CR #2 核验 |
|---|--------|------------|------------|
| 1 | 流式 append | ✅ | ✅ 已修复（见 `#2-1` 边界说明） |
| 2 | 权限 `optionId` | ✅ | ⚠️ 字段已改；选项逻辑仍有问题（`#2-6`） |
| 3 | 连接失败重试 | ✅ | ❌ **未真正修复**（`#2-2`） |
| 4 | placeholder 引用 | ✅ | ❌ **引入新 bug**（`#2-3`） |
| 5 | in-flight 输入锁 | ✅ | ✅ 已修复 |
| 6 | thought 与正文混显 | ✅ | ⚠️ 不再污染正文，但 thought 展示不完整（`#2-7`） |
| 7 | error 输入状态 | ✅ | ✅ 已修复 |
| 8 | initialize 握手 | ⏳ 称 SDK 内部处理 | ⚠️ **仍无显式 initialize**；若 Hermes 严格要求则有风险（`#2-8`） |
| 9 | Hermes 路径探测 | ✅ | ⚠️ 部分改进；**仍无 Settings 配置**（`#2-9`） |
| 10 | 进程 exit 僵尸 session | ✅ | ⚠️ AcpClient 内清理有改善；**Provider 层未同步**（`#2-4`） |
| 12 | cancel / 权限 | ✅ 部分 | ⚠️ cancel 未闭环（`#2-5`） |
...
```
WebView sendMessage
  → 禁用输入（无 streamEnd 则卡死 ⚠ #2-3）
  → Provider._handleUserMessage (fire-and-forget)
  → AcpClient.sendMessage
       → _responseBuffer = ''
       → session.prompt() …
       → session/update → buffer += chunk → addMessage(完整累积) ✅
       → prompt 完成 → streamEnd → 恢复输入 ✅

失败路径：
  start() 失败 → status error，_acp 仍占用 ⚠ #2-2
  process exit → stop() 异步 + status 竞态 ⚠ #2-4
  cancel → 无 streamEnd ⚠ #2-5

New Chat：
  innerHTML → const placeholder 赋值 💥 #2-1
```
...
| PLAN 能力 | 状态 |
|-----------|------|
| Phase 1 基础聊天 | ⚠️ 主路径可用，边界与 New Chat 仍有 bug |
| Phase 1 新建会话 | ⚠️ 功能有，实现有 P0 缺陷 |
| Phase 2 中断响应 | ⚠️ 有按钮，未闭环 |
